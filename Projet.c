...
void Lire_Info(stProduit *Produit){
    printf("Entrez l'identifiant de Produit : \n ");
    scanf(" %d",&(Produit->id));
...
void Print_Info(stProduit *ptr){
    printf("Id : %-10d Nom : %-29s   Prix : %-10.2f  Quantite : \t%-7d \n",ptr->id,ptr->nom,ptr->prix,ptr->quantite);
}
...
void charger(stProduit *Table[Taille_Table]){
    ...
    while(1){
        ...
        if(fscanf(pf,"%d|%29[^|]|%f|%d",&ptr->id,ptr->nom,&ptr->prix,&ptr->quantite)==4){
